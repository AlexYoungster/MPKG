...
class EDC:
    ...
    def load_model(self, model_name, model_type):
        ...
        if model_key in self.loaded_model_dict:
            logger.info(f" [1-SUCCESS]..{model_name} Model ------from {cache_dir}/-Loading MPKG Model...........")
            return self.loaded_model_dict[model_key]
        else:
            logger.info(f" [2-SUCCESS]...从{cache_dir}/MPKG/当中正在加载模型{model_name} -Loading model................")
            if model_type == "hf":
                ...
                logger.info(f" [3-SUCCESS]...{model_name} Model ------加载模型完毕- Model loaded successfully.........")  # <-- 新增: 日志信息
                # model, tokenizer = (
                #     AutoModelForCausalLM.from_pretrained(model_name, device_map="auto",cache_dir=cache_dir,  quantization_config=quantization_config, torch_dtype=torch.float16),
                #     AutoTokenizer.from_pretrained(model_name, cache_dir=cache_dir),
                #     logger.info(f" {model_name} Model ------正在使用8bit量化.")
                # )




                self.loaded_model_dict[model_key] = (model, tokenizer)
            elif model_type == "sts":
                from sentence_transformers import SentenceTransformer
                import torch
    ...
    #         cache_dir = '/root/autodl-tmp/models/e5'
    #     # 为E5模型设置特定的环境变量
    ...
    #     # 生成一个唯一的键，包含模型名称和缓存路径
    # model_key = f"{model_name}_{cache_dir}"
    
    # if model_key in self.loaded_model_dict:
    #     logger.info(f"Model {model_name} from {cache_dir} is already loaded, reusing it.")
    # else:
    #     logger.info(f"Loading model {model_name} from {cache_dir}")
    #     if model_type == "hf":
    #         model, tokenizer = (
    #             AutoModelForCausalLM.from_pretrained(model_name, device_map="auto", cache_dir=cache_dir),
    #             AutoTokenizer.from_pretrained(model_name, cache_dir=cache_dir),
    #         )
    #         self.loaded_model_dict[model_key] = (model, tokenizer)
    #     elif model_type == "sts":
    #         # 添加对sentence transformer的支持
    #         from sentence_transformers import SentenceTransformer
    #         model = SentenceTransformer(model_name, cache_folder=cache_dir)
    #         self.loaded_model_dict[model_key] = (model, None)  # SentenceTransformer不需要单独的tokenizer
    ...
    def schema_definition(self, input_text_list: List[str], oie_triplets_list: List[List[str]], free_model=False):
        ...
        if not llm_utils.is_model_openai(self.sd_llm_name):
            ...
            logger.info(f"Definition阶段正在启动........-Definition phase is initiating...")
        else:
        ...
        for idx, oie_triplets in enumerate(tqdm(oie_triplets_list)):
            # 添加日志：记录提示模板和示例
            logger.info(f"[SUCCESS] Schema Definition-Input loaded successfully: {input_text_list[idx][:80]}...")
            logger.info(f"[SUCCESS] Schema Definition-Triple extraction finished. Results below:: {oie_triplets}")
             # 添加日志：记录提示模板和示例
            # logger.info(f"Schema Definition 提示模板前几行: {schema_definition_few_shot_prompt_template_str[:100]}...")
            # logger.info(f"Schema Definition 示例前几行: {schema_definition_few_shot_examples_str[:100]}...")
            ...
            # 添加日志：记录输出
            logger.info(f"Schema Definition 输出: {schema_definition_dict}")
    ...
    def schema_canonicalization(
        ...
        logger.info("Running Schema Canonicalization正在执行schema标准化模块...")
        print("\n[DEBUG] ========== 开始Schema Canonicalization过程 ==========")
        sc_verify_prompt_template_str = open(self.sc_template_file_path).read()
        # print(f"[DEBUG] 验证提示模板前几行:\n{sc_verify_prompt_template_str[:200]}...")
        ...
        # 这是添加的关系   
        print(f"[DEBUG] Number of schemas in standard target schema: {len(self.schema)}")
        # print(f"[DEBUG] 目标模式中的部分关系示例:")
        for i, (rel, def_) in enumerate(list(self.schema.items())[:5]):
            print(f"  - '{rel}': {def_}")
        if len(self.schema) > 5:
            print(f"  - ... 还有 {len(self.schema) - 5} 个关系")
